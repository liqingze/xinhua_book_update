...
class PayService():
    ...
    def createOrder(self, member_id, items=None, params=None):
        ...
        # 悲观锁
        try:
            tmp_book_list = db.session.query(Book).filter(Book.id.in_(books_id)) \
                .with_for_update().all()
            tmp_book_stock_mapping = {}
            for tmp_item in tmp_book_list:
                tmp_book_stock_mapping[tmp_item.id] = tmp_item.stock
            ...
            for item in items:
                ...
                if int(item['number']) > int(tmp_left_stock):
                    raise Exception("您购买的这本书太火爆了， 剩余：%s，您购买：%s" % (tmp_left_stock, item['number']))
    ...
    def closeOrder(self, pay_order_id=0):
        ...
        if pay_order_items:
            for item in pay_order_items:
                ...
                if tmp_book_info:
                    tmp_book_info.stock = tmp_book_info.stock + item.quantity
